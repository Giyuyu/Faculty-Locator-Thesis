...
if exist "data\local-firebase\firebase-export-metadata.json" (
  call npx firebase emulators:start --project sti-locator-local --only %EMULATORS% --import "data\local-firebase" --export-on-exit "data\local-firebase"
) else (
  call npx firebase emulators:start --project sti-locator-local --only %EMULATORS% --export-on-exit "data\local-firebase"
)
