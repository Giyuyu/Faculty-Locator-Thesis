...
import { cacheAuthenticatedSession, loadAuthenticatedSession } from '../../utils/authSession';
...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      try {
        const sessionUser = await loadAuthenticatedSession(userCredential.user);
        cacheAuthenticatedSession(sessionUser);
        navigate(sessionUser.passwordChangeRequired ? '/profile?password=required' : '/home');
      } catch (sessionError) {
        await signOut(auth).catch(() => {});
        if (sessionError.message === 'inactive-account') {
          setError('Your account is deactivated or archived. Please contact the administrator.');
        } else {
          setError('Your authenticated account is not linked to an active system user.');
        }
      }
    } catch (err) {
